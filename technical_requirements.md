...
образования (далее Приложение). Приложение помогает во взаимодействии иностранных Студентов и их сопровождающих.  Также в приложении Студенты в доступном и понятном виде, на их языке, могут просмотреть и прочитать информацию, необходимую для первичной адаптации на территории России.
...
(ИС, Студент), Сопровождающий и Тимлидер. 
...
* Сопровождающий с помощью приложения может выбрать для себя приезд, который 
  хочет сопровождать. По каждому приезду имеет чек-лист необходимых 
  мероприятий и инстанций, которые нужно пройти вместе с ИС.
* Тимлидер является менеджером по взаимодействию Сопровождающего и ИС. 
  Имеет возможность убирать Сопровождающих из приезда (при конфликтных 
  ситуациях), утверждать состав Сопровождающих на приезд, подтверждать статус 
  профиля Сопровождающего, а также отвечать на вопросы ИС в поддержке.

## 2.2. Процесс регистрации, авторизации и аутентификации 

Регистрация всех типов пользователей осуществляется в приложении. По 
умолчанию регистрация происходит для ИС. Для Сопровождающих и Тимлидеров 
имеется возможность зарегистрироваться как Сопровождающий. Тимлидеры 
регистрируются как Сопровождающие, затем тип их профиля меняется вручную на 
сервере.
...
### 2.2.11. Выбор языка
Пользователю предлагается язык на выбор. Изначально весь текст в приложении отображается на русском языке и 
выбран русский язык. Если пользователь желает сменить язык, он выбирает нужный.

На экране отображается 1 кнопка:
1. Кнопка "Далее" — открывает экран 2.2.6. "Регистрация Сопровождающего".
...
1. Кнопка "Зарегистрироваться" — применяет выбор языка, открывает экран 2.2.3. "Регистрация ИС".
...
2. Email — введенное значение после окончания этапа регистрации заполняет поле 5.2. "Email" 
   в профиле ИС.
3. Пароль
...
На экране отображаются 3 кнопки — "Зарегистрироваться" и "Я Сопровождающий", "Войти":
1. Кнопка "Зарегистрироваться" — по нажатии на кнопку происходит:
   1. проверка пароля в поле 3 на соответствие требованиям. Если пароль не соответствует требованиям к 
      паролю, поле 3 подсвечивается красным по контуру, появляется надпись: "Пароль не соответствует требованиям".
   ...
   3. проверка валидности адреса электронной почты. Если почта не соответствует формату, то поле 2
...
3. Тип пользователя "Тимлидер" — по умолчанию открывается экран 2.4.2. "Список задач"
...
11. Последняя сопровождающая организация — заполняется автоматически, когда Сопровождающий выбирает приезд, в 
    котором есть этот ИС. Информация берется из поля Организация профиля Сопровождающего.
12. Последний сопровождающий — заполняется автоматически, когда Сопровождающий выбирает приезд, в котором есть этот 
    ИС. Информация берется из поля Полное имя профиля Сопровождающего.
13. Институт — выпадающий список со всеми институтами УрФУ (сортированный по количеству обучающихся там ИС) + 
    "Подготовительный факультет". Поле не отображается, если ИС обучается не в УрФУ.
14. Направление обучения — текстовое поле.
15. Дата последнего приезда — берется из поля "Дата приезда" последнего приезда, к которому относится Студент. 
...
16. Дата окончания последней визы
17. Место проживания — текстовое поле.
18. Комментарий от сопровождающего — текстовое поле.
...
Внизу экрана отображаются 3 кнопки — "Добавить больше участников приезда" и "Отправить информацию о приезде", "Загрузить билеты":
...
1. ИС выбрал организацию PRIVET, но еще не оплатил сопровождение (поле 10. "Статус оплаты сопровождения" в профиле ИС имеет значение "Нет"), то ИС имеет ограниченный доступ на этом экране.
...
Если статус аккаунта Сопровождающего является не подтвержденным (поле 10 в профиле Сопровождающего), то на 
...
0. Фото — круглая область с фотографией, по умолчанию стоит заглушка. При тапе на область открывается системный 
   файлпикер, загружается изображение, обрезается до формата 1x1, вписывается в круг.
1. Организация — организация, в которой работает Сопровождающий.
2. Университет — университет, в котором в данный момент обучается Сопровождающий.
3. Город — город, в котором обучается и проживает Сопровождающий.
4. Полное имя
5. Дата рождения — справа от даты прозрачными цифрами отображается число полных лет Сопровождающего.
6. Контакты:
   1. Телефон — В начале поля стоит полупрозрачный "+" в виде 
       плейсхолдера. Само поле имеет формат цифр. Длина до 12 
       символов.
...
8. Другие языки и уровень владения ими — аналогично полю 7.
   ...
   языком по классификации CERF.
9. Тип профиля — имеет значения "Студент", "Сопровождающий" и "Тимлидер". Для Сопровождающего выбирается статус 
   "Сопровождающий" по умолчанию при регистрации. Может быть изменено администратором на "Тимлидер". 
10. Статус аккаунта Сопровождающего — поле, отображающее статус подтверждения аккаунта Сопровождающего. Новый 
...
Все поля, кроме 10-11, должны быть заполнены. Поля 10-11 могут быть изменены только тимлидером.
